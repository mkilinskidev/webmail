					...
					fAddAllAttachmentsDownloadMethod({
						...
						'Handler': function (accountId, hashes, attachments) {
							const
								...
								SelectFilesPopup = __webpack_require__(/*! modules/MailSaveAttachmentsToFilesPlugin/js/popups/SelectFilesPopup.js */ "zu1B")
							;
...
CSharePopup.prototype.onCreatePublicLinkResponse = function (response)
{
	if (response.Result) {
		...
		Api.showErrorByCode(response, TextUtils.i18n('FILESWEBCLIENT/ERROR_CREATE_PUBLIC_LINK'));
	}
};
...
CSharePopup.prototype.onCancelSharingClick = function ()
{
	if (this.item)
	{
		Ajax.send('DeletePublicLink', {
			...
			if (!response.Result) {
				Api.showErrorByCode(response, TextUtils.i18n('FILESWEBCLIENT/ERROR_DELETE_PUBLIC_LINK'));
			} else {
				this.closePopup();
			}
		}, this);
...
CSharePopup.prototype.showHistory = function () {
	...
	{
		Popups.showPopup(ShowHistoryPopup, [TextUtils.i18n('FILESWEBCLIENT/HEADING_HISTORY_POPUP'), this.item]);
	}
}
	...
	this.fileListInfoText = ko.computed(function () {
		...
		else if (this.loadedFiles())
		{
			if (this.collection().length === 0)
			{
				...
				{
					sInfoText = TextUtils.i18n('FILESWEBCLIENT/INFO_SHARED_FOLDER_IS_EMPTY');
				}
...
CFilesView.prototype.dragAndDropHelper = function (oDraggedItem, ctrlOrCmdUsed)
{
	...
	if (!oCounts.file) {
		sText = TextUtils.i18n('FILESWEBCLIENT/LABEL_DRAG_FOLDERS_PLURAL', {'COUNT': sPlusPrefix + oCounts.folder}, null, oCounts.folder);
	} else if (!oCounts.folder) {
		sText = TextUtils.i18n('FILESWEBCLIENT/LABEL_DRAG_FILES_PLURAL', {'COUNT': sPlusPrefix + oCounts.file}, null, oCounts.file);
	} else {
		sText = TextUtils.i18n('FILESWEBCLIENT/LABEL_DRAG_ITEMS_PLURAL', {'COUNT': sPlusPrefix + aItems.length}, null, aItems.length);
	}
	...
	if ((oParameters.Type === this.storageType() || oParameters.Hash === Settings.PublicHash) && oParameters.Path === this.currentPath())
	{
		...
		else
		{
			if (oResponse.ErrorCode !== Enums.Errors.NotDisplayedError)
			{
				this.loading(false);
				this.error(true);
				Api.showErrorByCode(oResponse, TextUtils.i18n('FILESWEBCLIENT/ERROR_FILES_NOT_RECEIVED'));
			}
		}
	}
};
...
CFilesView.prototype.executeDelete = function ()
{
	...
	if (!this.bPublic && itemsToDeleteCount > 0) {
		...
		if (askAboutSharedItems) {
			confirmText = TextUtils.i18n('FILESWEBCLIENT/CONFIRM_NOT_ALL_ITEMS_OWN');
		} else if (hasFolder && hasFile) {
			confirmText = TextUtils.i18n('FILESWEBCLIENT/CONFIRM_DELETE_ITEMS_PLURAL', {'COUNT': itemsToDeleteCount}, null, itemsToDeleteCount);
		} else if (hasFolder) {
			confirmText = TextUtils.i18n('FILESWEBCLIENT/CONFIRM_DELETE_FOLDERS_PLURAL', {'COUNT': itemsToDeleteCount}, null, itemsToDeleteCount);
		} else {
			confirmText = TextUtils.i18n('FILESWEBCLIENT/CONFIRM_DELETE_FILES_PLURAL', {'COUNT': itemsToDeleteCount}, null, itemsToDeleteCount);
		}
